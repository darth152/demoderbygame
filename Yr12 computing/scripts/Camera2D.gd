...
func _process(_delta):
	if global.screen_shake != 0: camera_shake()
...
func get_camera_pos(): #Find camera position
	...
	c_mouse_pos.x = ((c_mouse_pos.x/c_mouse_reach) + global.GLOBAL_p_pos.x) + rand_range(shake, shake * -1) #Go to player X-Axis + mouse position
	c_mouse_pos.y = ((c_mouse_pos.y/c_mouse_reach) + global.GLOBAL_p_pos.y) + rand_range(shake, shake * -1) #Same as above with Y-Axis


func get_camera_zoom(): #Find camera zoom
	#Using Pythagorus to find the distance the player is from the mouse on an angle
	var c_pos = Vector2(c_mouse_pos.x - global.GLOBAL_p_pos.x, c_mouse_pos.y - global.GLOBAL_p_pos.y)
...
func camera_shake(): #Apply screenshake
	if global.shake_mode == false: shake = global.screen_shake
	else: shake = global.screen_shake / 3
